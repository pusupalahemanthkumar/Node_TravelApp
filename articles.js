export default [
  {
    "title": "Travel Safety",
    "image": "data:image/jpeg;base64,",
    "shortDescription": "With growing numbers of Americans getting vaccinated and even boosted against COVID-19, travel is feeling a lot safer than it did last holiday season.If you’re fully vaccinated, you’re not taking as much of a risk if you travel now. You still need to take some precautions, but it’s different than it was in the past,” says Aaron E. Glatt, MD, chairman of the department of medicine and chief of infectious diseases at Mount Sinai South Nassau in Oceanside, New York.",
    "category": ["Tips"],
    "content": [
      {
        "title": " Know the COVID-19 Rate Where You Live",
        "description": "Even if you’re vaccinated, it’s still important to assess COVID-19 levels in you community. If they’re high, you are at greater risk of developing COVID-19, though these so-called breakthrough infections are typically mild.Vaccination is excellent in protecting people, but it is not perfect and we do see a small number of vaccinated persons developing a breakthrough infection. Thankfully, these infections tend to be less severe,” says Richard Martinello, MD, an infectious diseases specialist at Yale Medicine and associate professor of medicine at the Yale School of Medicine in New Haven, Connecticut."
      },
      {
        "title": "Assess COVID-19 Rates at Your Destination",
        "description": "The infection rate at your destination is also a factor to consider. If you’re heading to a location that’s red-hot with COVID-19, your chances of becoming ill there do rise, even if you’ve been vaccinated.For travel within the United States, you can find an area’s test positivity rate (a key measure of virus circulation levels) on the website of the local public health department or in the CDC’s community database (click “your community,” then “county view”). Or search for the color-coded risk level at the comprehensive website Global Epidemics from the Brown School of Public Health."
      },
      {
        "title": "Vaccination Status and Health Situation",
        "description": "A key question to ponder is how risky would traveling be for you. “Everything with COVID-19 needs to be individualized. You need to view everything through your personal perspective,” Glatt says.Are you unvaccinated and at high risk for severe COVID-19 consequences because of your age or underlying health conditions? And what about the people you’re traveling with or visiting? Perhaps they have a compromised immune system and remain vulnerable to COVID-19 even if they’ve been vaccinated."
      },
      {
        "title": "Think About Testing Around Your Trip",
        "description": "If you or those you’re traveling with have not been vaccinated, you may want to take a COVID-19 test one to three days before you leave in order to reduce the odds you’ll unknowingly bring the virus to your destination.In addition, the CDC recommends unvaccinated people test again three to five days after traveling, and also quarantine for seven days (or 10 days if skipping the post-trip test).Some international destinations require testing before you can visit, even if you’ve been vaccinated. For instance, Brazil allows U.S. citizens to enter only if they’ve tested negative for COVID-19 before arrival."
      },
      {
        "title": "Decide on Your Mode of Travel",
        "description": "For anyone who isn’t vaccinated or is at high risk of severe COVID-19, experts consider driving to be the safest form of transportation, especially if the destination can be reached within a day, because this substantially limits interactions with other people.Flying can also be relatively safe. As of now, airlines continue to require all passengers to wear masks onboard. If you are unvaccinated be sure to remain in your seat as much as possible during the flight and keep your mask on nearly all the time, especially when other passengers nearby remove theirs to eat or drink."
      }
    ]
  },
  {
    "title": "About Taj Exotica",
    "image": "https://cdn.dribbble.com/users/1712260/screenshots/5823675/comp_5.gif",
    "shortDescription":"Built with a style that reflects the culture of the local tribes, Taj Exotica Resort and Spa is one of the best resorts in Andaman located on Radhanagar beach. The estate stretches across 46 acres of land and is covered by a dense canopy of mangroves. The rooms and villas are a confluence of the local culture and modernity, fitted with all comforts. The signature in-house eateries serve sumptuous exotic delicacies for its guests. Enjoy adventurous activities like Snorkeling, Scuba-diving and kayaking to unravel the secrets of this mystical island.",
    "category": ["Tips"],
    "content": [
      {
        "title": "TITLE-1",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-2",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-3",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-4",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      }
    ]
  },
  {
    "title": "Travel Guide",
    "image": "https://media.istockphoto.com/id/1280363533/photo/female-translator-working-on-a-document.jpg?b=1&s=170667a&w=0&k=20&c=7QcP6C7dhb85GMvy6smvFAXKeAXlHtKRKKgOJlkTYJM=",
    "shortDescription":"Built with a style that reflects the culture of the local tribes, Taj Exotica Resort and Spa is one of the best resorts in Andaman located on Radhanagar beach. The estate stretches across 46 acres of land and is covered by a dense canopy of mangroves. The rooms and villas are a confluence of the local culture and modernity, fitted with all comforts. The signature in-house eateries serve sumptuous exotic delicacies for its guests. Enjoy adventurous activities like Snorkeling, Scuba-diving and kayaking to unravel the secrets of this mystical island.",
    "category": ["Tips"],
    "content": [
      {
        "title": "TITLE-1",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-2",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-3",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-4",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      }
    ]
  },
  {
    "title": "Islands",
    "image": "https://res.cloudinary.com/dtpgi0zck/image/upload/s---HMVVs4u--/c_fit,h_580,w_860/v1/EducationHub/photos/sea-level.jpg",
    "category": ["Experience"],
    "shortDescription":"Built with a style that reflects the culture of the local tribes, Taj Exotica Resort and Spa is one of the best resorts in Andaman located on Radhanagar beach. The estate stretches across 46 acres of land and is covered by a dense canopy of mangroves. The rooms and villas are a confluence of the local culture and modernity, fitted with all comforts. The signature in-house eateries serve sumptuous exotic delicacies for its guests. Enjoy adventurous activities like Snorkeling, Scuba-diving and kayaking to unravel the secrets of this mystical island.",
    "content": [
      {
        "title": "TITLE-1",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-2",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-3",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-4",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      }
    ]
  },
  {
    "title": "Hill Stations",
    "image": "https://static2.tripoto.com/media/filter/tst/img/4335/TripDocument/1471437626_famous_hill_stations_in_india.jpg",
    "shortDescription":"Built with a style that reflects the culture of the local tribes, Taj Exotica Resort and Spa is one of the best resorts in Andaman located on Radhanagar beach. The estate stretches across 46 acres of land and is covered by a dense canopy of mangroves. The rooms and villas are a confluence of the local culture and modernity, fitted with all comforts. The signature in-house eateries serve sumptuous exotic delicacies for its guests. Enjoy adventurous activities like Snorkeling, Scuba-diving and kayaking to unravel the secrets of this mystical island.",
    "category": ["Experience"],
    "content": [
      {
        "title": "TITLE-1",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-2",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-3",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-4",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      }
    ]
  },
  {
    "title": "Beach",
    "image": "https://www.travelandleisure.com/thmb/HlNYcpqWt9t1IgQq1eTgJG3hp6k=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/saud-beach-luzon-philippines-WRLDBEACH0421-15e2c368e7ad4495be803bd60cafa379.jpg",
    "shortDescription":"Built with a style that reflects the culture of the local tribes, Taj Exotica Resort and Spa is one of the best resorts in Andaman located on Radhanagar beach. The estate stretches across 46 acres of land and is covered by a dense canopy of mangroves. The rooms and villas are a confluence of the local culture and modernity, fitted with all comforts. The signature in-house eateries serve sumptuous exotic delicacies for its guests. Enjoy adventurous activities like Snorkeling, Scuba-diving and kayaking to unravel the secrets of this mystical island.",
    "category": ["Experience"],
    "content": [
      {
        "title": "TITLE-1",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-2",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-3",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      },
      {
        "title": "TITLE-4",
        "description": "Endowed with a old-world charm, this polished resort flaunts ornate Mediterranean architecture steeped in tropical modernism. Guests can choose from beach villas with sylvan patios and spacious suites having latest amenities. Take a rejuvenating plunge at the pool and relax at the garden. The resort houses a fitness centre, spa, golf course and international restaurant."
      }
    ]
  }
  
]
